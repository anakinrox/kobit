function zoom(componente, idCampoTexto) {
	...
	if (campo == 'bt_solic') {
		modalzoom.open("Solicitante",
			"solicitantes",
			"login,Cod.,nome_usuario,Nome",
			"login,nome_usuario,cod_secao,den_secao,matricula",
			"",
	...
	if (campo == 'bt_secao') {
		modalzoom.open("Seção",
			"selectTableSQLserver",
			"codigo,Código,descricao,Descrição",
			"codigo,descricao",
			"dataBase,java:/jdbc/FluigDS,table,[RM].[CorporeRM].[dbo].[PSECAO],LEN(codigo),16",
			componente, false, 'default', null, null,
			"codigo||'-'||descricao");
	}

	if (campo == 'bt_tipo') {
		modalzoom.open("Tipo de Chamado",
			"tipo_solicitacao",
			"cod_tipo,Cod.,den_tipo,Den Tipo,descricao,Detalhamento",
			"cod_tipo,den_tipo,aprovacao_n1,aprovacao_n2,pontos_tipo,descricao",
			"",
			componente, false, 'default', null, null,
			"den_tipo");
	}
	...
	if (campo == 'bt_setor') {
		modalzoom.open("Setor Responsável",
			"setor",
			"cod_setor,Cod.,den_setor,Descrição",
			"cod_setor,den_setor,cod_papel_deploy,cod_papel_n1,cod_papel_n2,matricula_deploy,matricula_n1,matricula_n2,pool_papel_deploy,pool_papel_n1,pool_papel_n2",
			"",
			componente, false, 'default', null, null,
			"den_setor");
	}
	...
	if (campo == 'bt_categoria') {
		...
		if ($('#cod_setor_' + $this.instanceId).val().trim() != '') {
			filtroCPL += ",cod_setor," + $('#cod_setor_' + $this.instanceId).val();
		};

		if ($('#cod_tipo_' + $this.instanceId).val().trim() != '') {
			filtroCPL += ",___like___sub_cod_tipo," + $('#cod_tipo_' + $this.instanceId).val() + '|x'
		};

		modalzoom.open("Categoria",
			"selectPaiFilho",
			"categoria,Categoria",
			"distinct,categoria,cod_categoria,cod_papel_deploy,cod_papel_n1,cod_papel_n2,pool_papel_deploy,pool_papel_n1,pool_papel_n2,matricula_deploy,matricula_n1,matricula_n2",
			"dataset,catalogo,table,tipos_solicitacoes" + filtroCPL,
			componente, false, 'default', null, null,
			"categoria");

	}

	if (campo == 'bt_subcategoria') {

		var filtroCPL = '';
		if ($('#cod_categoria_' + $this.instanceId).val().trim() != '') {
			filtroCPL += ",cod_categoria," + $('#cod_categoria_' + $this.instanceId).val();
		};

		if ($('#cod_tipo_' + $this.instanceId).val().trim() != '') {
			filtroCPL += ",___like___sub_cod_tipo," + $('#cod_tipo_' + $this.instanceId).val();
		};

		modalzoom.open("Subcategoria",
			"selectPaiFilho",
			"desc_subcategoria,Subcategoria",
			"distinct,desc_subcategoria",
			"dataset,catalogo,table,sub_categoria" + filtroCPL,
			componente, false, 'default', null, null,
			"desc_subcategoria");

	}

	if (campo == 'bt_atividade') {

		modalzoom.open("Atividade",
			"dsk_processTask",
			"cod_atividade,Atividade,den_atividade,Des Atividade",
			"cod_atividade,den_atividade",
			"cod_processo,Chamado",
			componente, false, "default", null, null,
			"cod_atividade||'-'||den_atividade");
...
function setSelectedZoomItem(selectedItem) {
	...
	if (campo == "bt_solic") {
		$('#cod_solicitante_' + $this.instanceId).val(selectedItem.login);
		$('#solicitante_' + $this.instanceId).val(selectedItem.nome_usuario);
	}

	if (campo == "bt_atend") {
		$('#cod_atendente_' + $this.instanceId).val(selectedItem.login);
		$('#atendente_' + $this.instanceId).val(selectedItem.nome_usuario);
	}

	if (campo == 'bt_secao') {

		$('#cod_secao_' + $this.instanceId).val(selectedItem.codigo);
		$('#secao_' + $this.instanceId).val(selectedItem.codigo + ' - ' + selectedItem.descricao);
	}

	if (campo == 'bt_tipo') {
		$('#cod_tipo_' + $this.instanceId).val(selectedItem.cod_tipo);
		$('#tipo_' + $this.instanceId).val(selectedItem.den_tipo);
	}

	if (campo == 'bt_setor') {
		$('#cod_setor_' + $this.instanceId).val(selectedItem.cod_setor);
		$('#setor_' + $this.instanceId).val(selectedItem.den_setor);
	}

	if (campo == 'bt_categoria') {
		$('#cod_categoria_' + $this.instanceId).val(selectedItem.cod_categoria);
		$('#categoria_' + $this.instanceId).val(selectedItem.categoria);
	}

	if (campo == 'bt_subcategoria') {
		// $('#cod_subcategoria'+ $this.instanceId).val(selectedItem.login);
		$('#subcategoria_' + $this.instanceId).val(selectedItem.desc_subcategoria);
	}

	if (campo == 'bt_atividade') {
		$('#cod_atividade_' + $this.instanceId).val(selectedItem.cod_atividade);
		$('#atividade_' + $this.instanceId).val(selectedItem.den_atividade);
	}
