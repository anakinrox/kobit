...
function exportToXLS(table, fileName, sheet) {
	const wb = XLSX.utils.book_new();
	wb.SheetNames.push(sheet);

	const tableHeader = Array.from(table.tHead.rows[0].querySelectorAll('th'))
		.filter(th => th.querySelector('input') == null)
		.map(th => th.innerText);

	const tableBody = Array.from(table.tBodies[0].rows)
		.map(row => {
	...
	const ws_data = [tableHeader, ...tableBody];
...
function exportToCSV(table, fileName) {
	// const universalBOM = '\uFEFF';
	const csvHeader = Array.from(table.tHead.rows[0].querySelectorAll('th'))
		.map(th => th.innerText)
		.join(';');

	const csvBody = Array.from(table.tBodies[0].rows)
