$(document).bind('DOMNodeRemoved', function (e) {
  ...
  if ($(target).hasClass('container-modal')) {
	if( $(".container-modal").length == 1 ){
...
var modalzoom = (function () {
  ...
  return {
    open: function (title, dataset, fields, resultfields, filters, type, iniVazio, size, likefield, likevalue, searchby) {
      if (size != 'none') parent.$('#workflowview-header').hide();
      ...
      if (size == '' || size == undefined || size == 'default') size = 'large';
      ...
      var html =
        "<body class='fluig-style-guide' id='modal-zoom' >" +
      ...
      var readydataset = function (dataset) {
        ...
        for (var i = 0; i < dataset.length; i++) {
          var row = dataset[i];
          linhas += 1;
          var html = '<tr class="fs-cursor-pointer" data-dataset=' + i + '>';
        ...
        $('#msg').remove();
        if (linhas == sqlLimit) {
        	$( "[id^='modal-zoom-'] .modal-footer" ).prepend(
            '<span id="msg"  name="msg" style="text-align: left;color: red;float: left;"><b> *Foram listados ' +
              sqlLimit +
              ' registros, refine sua busca!</b></span>'
          );
        }
        loading.hide();
      };
