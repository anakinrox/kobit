...
$(document).bind("DOMNodeRemoved", function(e){
	var target = $(e.target);
	if( target.html().indexOf("id='modal-zoom-" ) > 0 || target.html().indexOf("id='modal-zoom'" ) ){
...
var modalzoom = (function(){
	...
	return {
		open: function(title, dataset, fields, resultfields, filters, type, iniVazio, size, likefield, likevalue, searchby) {
			...
			//parent.$('#workflowView-cardViewer').css( 'zIndex', 9999 );
			//parent.$('#workflowView-cardViewer').css( 'zIndex', 1 );
			parent.$('#workflowview-header').hide();
			//workflow-view-tabs workflow-collapse-tabs-static
...
/*			var cplHtml = '';
			...
			var html = "<body class='fluig-style-guide' id='modal-zoom' >" +
					...
					var readydataset = function(dataset) {
						...
						//gambi
						if ( dataset.length == 1 && ( size == 'none' || size == 'more') ){
						...
						console.log('Passei if.......', dataset.length);
						if ( size == 'more' ){
							size = "large";
							console.log('Ajusta tamanho.......');
						}
