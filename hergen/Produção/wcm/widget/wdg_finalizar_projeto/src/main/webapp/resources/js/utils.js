...
function f_aplicaPerce(idObjValor, idObjPerc, idObjResult, indAgragar, idObjMultiplo) {

    if ($('#' + idObjPerc).val() == "" || $('#' + idObjValor).val() == "") {
        return
    }
    var wValor = $('#' + idObjValor).val().toString().replace(".", "");

    wValor = parseFloat(wValor.toString().replace(",", "."));
    var wCalc = (wValor * parseInt($('#' + idObjPerc).val()) / 100);

    if (indAgragar != null && indAgragar != undefined && indAgragar == 'S') {
        wCalc = formatDecimal(((wCalc) + wValor), 4);
    } else {
        if (idObjMultiplo != undefined) {
            var quant = wCalc;
            var wQtd_multi = parseInt($('#' + idObjMultiplo).val());

            if ((quant % wQtd_multi) != 0) {
                wValor = parseFloat(wValor.toString().replace(",", "."));
                wCalc = Math.floor(((wValor * parseInt($('#' + idObjPerc).val()) / 100) / wQtd_multi));
                wCalc = (parseInt(wCalc) * wQtd_multi);
            }
        }
        wCalc = formatDecimal(wCalc, 0);
    }

    // Retorno valor
    $('#' + idObjResult).val(wCalc);
    return
