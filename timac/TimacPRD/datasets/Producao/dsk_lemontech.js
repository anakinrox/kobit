...
function onSync(lastSyncDate) {
    try {
        ...
        // envelope += '     <ser:pesquisarSolicitacao>';
        // envelope += '     <version>4.0</version>';
        // envelope += '     <idSolicitacaoRef>10479575</idSolicitacaoRef>';
...
function f_criaDocumento(objFoto, obj, folderID, seq) {
    ...
    for (var x = 0; x < wLengthFotos; x++) {
        ...
        var idDocumento = f_gravaArquivo(folderID, "marcio.kobit", "Teste", fileName, obj.aprovador.nome, wExtencao);
    ...
    try {
        ...
        while (rsWD.next()) {
            ...
            if (rsWD.getString("status").trim() != pStatus.trim()) {
                var sqlUPD = "update kbt_t_lemontech set dat_registro = GETDATE() ";
        ...
        if (wIncluido == false) {
            var sqlUPD = "insert into kbt_t_lemontech (numSV, status, integrado, dat_registro) values (" + pNumSV + ",'" + pStatus + "','" + pIntegrado + "', GETDATE())";
            var statementWD = connectionWD.prepareStatement(sqlUPD);
            statementWD.executeUpdate();
        }
    } catch (error) {
