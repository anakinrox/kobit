...
function f_getLicencas(newDataset) {
	try {
		var retorno = null;
		var metodo = "GET";
		var wServiceCode = "fluigApi";
		var params = {};
		var Endpoint = "/license/api/v1/licenses";
		...
		if (vo.getResult() == "" || vo.getResult().isEmpty()) {
			// newDataset.addRow(new Array("Retorno esta vazio"));
			// 	// throw "Retorno esta vazio";
		} else {
			var jr = JSON.parse(vo.getResult());
			// newDataset.addRow(new Array(vo.getResult()));
			retorno = jr;
		}
		// newDataset.addRow(new Array("Aqui "));

	} catch (err) {
		newDataset.addRow(new Array("Error: " + err));
...
function f_montaResumo(newDataset) {
	try {
		...
		var objLicensas = f_getLicencas(newDataset);
		var nTotalLicances = (objLicensas != null ? objLicensas.items[0].totalLicenses : 0);
		...
		wStringTabela += '	</tr> ';
		wStringTabela += '	<tr> ';
		wStringTabela += '		<td colspan="4" style="text-align: center"><b><h2>Relação de usuários</h2></b></td> ';
		wStringTabela += '	</tr> ';
		wStringTabela += '	<tr> ';
		wStringTabela += '		<td><b>Total</b></td> ';
		wStringTabela += '		<td><b>Ativos</b></td> ';
		wStringTabela += '		<td colspan="2" style="text-align: center"><b>Bloqueados</b></td> ';
		wStringTabela += '	</tr> ';
		wStringTabela += '	<tr> ';
		wStringTabela += '		<td>' + nTotalLicances + '</td> ';
		...
		f_processoTrocaUsuario(newDataset);
	}


}

function f_processoTrocaUsuario(newDataset) {
	try {
		var gson = new com.google.gson.Gson();
		var wUsuarios = [];
		var wErro = false;

		// newDataset.addRow(new Array(gson.toJson(arrUsuarioDesativados)));
		for (var i = 0; i < arrUsuarioDesativados.length; i++) {
			if (arrUsuarioDesativados[i].status = 'ERROR') {
				wErro = true;
				var data = {
					matricula: arrUsuarioDesativados[i].matricula,
					nome: arrUsuarioDesativados[i].nome,
					processos: arrUsuarioDesativados[i].processosUsuario
				}
				wUsuarios.push(data);
			}
		}
		...
		if (wErro) {
			...
			constraints.push(DatasetFactory.createConstraint('processo', 'inativar_usuario', null, ConstraintType.MUST));
			constraints.push(DatasetFactory.createConstraint('atividade', '14', null, ConstraintType.MUST));
			// constraints.push(DatasetFactory.createConstraint('usuario', 'admin', null, ConstraintType.MUST));
			constraints.push(DatasetFactory.createConstraint('data_disparo', dataAtualFormatada(), 'field', ConstraintType.MUST));

			for (var i = 0; i < wUsuarios.length; i++) {
				for (var x = 0; x < wUsuarios[i].processos.length; x++) {
					constraints.push(DatasetFactory.createConstraint('cod_matricula_atual___' + (x + 1), wUsuarios[i].matricula, 'field', ConstraintType.MUST));
					constraints.push(DatasetFactory.createConstraint('nome_usuario_atual___' + (x + 1), wUsuarios[i].nome, 'field', ConstraintType.MUST));

					constraints.push(DatasetFactory.createConstraint('cod_processo___' + (x + 1), wUsuarios[i].processos[x].codprocesso, 'field', ConstraintType.MUST));
					constraints.push(DatasetFactory.createConstraint('nome_processo___' + (x + 1), wUsuarios[i].processos[x].nomprocesso, 'field', ConstraintType.MUST));

					constraints.push(DatasetFactory.createConstraint('cod_matricula_novo___' + (x + 1), '', 'field', ConstraintType.MUST));
					constraints.push(DatasetFactory.createConstraint('nome_usuario_novo___' + (x + 1), '', 'field', ConstraintType.MUST));
				}
			}
			printLog("info", "Const Inativar " + gson.toJson(constraints));
			var dsProcesso = DatasetFactory.getDataset('processo_movimento', null, constraints, null);
			printLog("info", "Processo Inativar " + dsProcesso);
		}
...
function f_incluirUsuario(newDataset) {
	...
	try {
		...
		// var SQL = "select * from eis_v_funcionarios_protheus_digte where MATRICULA = '99-900293'";
...
function f_desativarUsuario(newDataset) {
	...
	try {
		...
		// 		var SQL = "select * from eis_v_funcionarios_protheus_digte where data_desligamento is not null and ies_cad_ativo = 'N' and cpf = '09042050969'";
		...
		while (rsWD.next()) {
			...
			if (dataSet.rowsCount > 0) {
				...
				if (dataSet.getValue(0, "active") == "true") {
					var wID = dataSet.getValue(0, "colleaguePK.colleagueId");
					// 	newDataset.addRow(new Array("USUARIO MATRICULA = -> " + wID + ' Data: ' + dataAtualFormatada()));
					try {
						var processosDoUsuario = f_getProcessoByColleagueId(wID, newDataset);
						log.dir(processosDoUsuario);
						var estaNosProcessosPermitidos = f_checaSeUsuarioEstaNosProcessosPermitidos(processosDocumentoArr, processosDoUsuario);
						newDataset.addRow(new Array("Estado = " + estaNosProcessosPermitidos + "nome: " + rsWD.getString("NOME_COMPLETO") + " CPF: " + rsWD.getString("CPF") + ' Situacao: ' + dataSet.getValue(0, "active")));
						if (estaNosProcessosPermitidos) {
							newDataset.addRow(new Array("PERMITIDO INATIVAR."));
							var wListaProcesso = [];
							for (p = 0; p < processosDoUsuario.length; p++) {
								var wAcho = false;
								for (v = 0; v < wListaProcesso.length; v++) {
									if (wListaProcesso[v].codprocesso == processosDoUsuario[p].processo) {
										wAcho = true;
										break;
									}
								}
								if (!wAcho) {
									var data = {
										codprocesso: processosDoUsuario[p].codprocesso,
										nomprocesso: processosDoUsuario[p].processo
									}
									wListaProcesso.push(data);
								}
							}
							...
							try {
								...
								var data = {
									...
									status: 'ERROR',
									processosUsuario: wListaProcesso
								};
								arrUsuarioDesativados.push(data);
								// newDataset.addRow(new Array("Error ao Inativar: " + error.toString()));
							}
						} else {
							newDataset.addRow(new Array("NAO PERMITIDO INATIVAR."));
							...
							for (p = 0; p < processosDoUsuario.length; p++) {
								var wAcho = false;
								for (v = 0; v < wListaProcesso.length; v++) {
									if (wListaProcesso[v].codprocesso == processosDoUsuario[p].codprocesso) {
										wAcho = true;
										break;
									}
								}
							...
							newDataset.addRow(new Array('Proc: ' + gson.toJson(wListaProcesso)));
						}
...
function f_getProcessoByColleagueId(colleagueId) {
	...
	try {
		...
		var SQL = "SELECT DISTINCT NUM_PROCES, COD_DEF_PROCES, des_def_proces ";
		SQL += " 	FROM (SELECT a.NUM_PROCES, a.COD_DEF_PROCES, d.des_def_proces ";
		SQL += " 			FROM PROCES_WORKFLOW  a, def_proces d ";
		SQL += " 			WHERE a.LOG_ATIV = 1  ";
		SQL += " 			AND a.STATUS = 0  ";
		SQL += " 			AND d.COD_DEF_PROCES = a.COD_DEF_PROCES ";
		SQL += " 			AND a.COD_MATR_REQUISIT = '" + colleagueId + "'	 ";
		SQL += " 			UNION	 ";
		SQL += " 			SELECT a.NUM_PROCES, a.COD_DEF_PROCES, d.des_def_proces	 ";
		SQL += " 			FROM TAR_PROCES  t ";
		SQL += " 			JOIN PROCES_WORKFLOW a ON (a.COD_EMPRESA = t.COD_EMPRESA AND a.NUM_PROCES = t.NUM_PROCES)  ";
		SQL += " 			JOIN def_proces d ON (d.COD_DEF_PROCES = a.COD_DEF_PROCES) ";
		SQL += " 			WHERE a.LOG_ATIV = 1  ";
		SQL += " 			AND a.STATUS = 0  ";
		SQL += " 			AND t.LOG_ATIV = 1  ";
		SQL += " AND t.CD_MATRICULA = '" + colleagueId + "') t ";
		...
		while (rsWD.next()) {
			var data = {
				codprocesso: rsWD.getString("COD_DEF_PROCES"),
				processo: rsWD.getString("des_def_proces"),
