...
function createDataset(fields, constraints, sortFields) {
	...
	try {
		...
		var data = {
			...
			endpoint: '/LOGIXREST/kbtr00005/cliente/' + cliente,
		...
		if (vo.getResult() == null || vo.getResult().isEmpty()) {
			...
			var wCodCidadeEnt = '';
			var wCepEnt = '';
			var wIndeLogradouroEnt = '';
			var wTipoLogradouroEnt = '';


			// log.dir(jr);
			for (var i = 0; i < jr.data.cli_end_ent.length; i++) {
				if (jr.data.cli_end_ent[i].tip_endereco_end_co == 'E') {
					wEndEnt = jr.data.cli_end_ent[i].logradouro_end_cob;
					wComplEndEnt = jr.data.cli_end_ent[i].complemento_endereco_end_cob;
					wBairroEnt = jr.data.cli_end_ent[i].bairro_cobr_entga;
					wCodCidadeEnt = jr.data.cli_end_ent[i].cod_cidade_end_cob;
					wCepEnt = jr.data.cli_end_ent[i].cod_cep_end_cob;
					wIndeLogradouroEnt = jr.data.cli_end_ent[i].num_iden_lograd_end_cob;
					wTipoLogradouroEnt = jr.data.cli_end_ent[i].tip_logradouro_end_cob;
				}

				if (jr.data.cli_end_ent[i].tip_endereco_end_co == 'C') {
					wEndCob = jr.data.cli_end_ent[i].logradouro_end_cob;
					wBairroCob = jr.data.cli_end_ent[i].bairro_cobr_entga;
					wCodCidadeCob = jr.data.cli_end_ent[i].cod_cidade_end_cob;
					wCepCob = jr.data.cli_end_ent[i].cod_cep_end_cob;
				}
			}
			...
			cli.push(wEndCob);
			cli.push(wBairroCob);
			cli.push(wCodCidadeCob);
			cli.push(wCepCob);

			for (var i = 0; i < jr.data.cli_canal_venda.length; i++) {
				if (jr.data.cli_canal_venda[i].cod_tip_carteira == '01') {
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_1);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_2);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_3);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_4);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_5);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_6);
					cli.push(jr.data.cli_canal_venda[i].cod_nivel_7);
					cli.push(jr.data.cli_canal_venda[i].ies_nivel);
					cli.push(jr.data.cli_canal_venda[i].cod_tip_carteira);
				} else {
					cli.push('');
					cli.push('');
					cli.push('');
			...
			cli.push(''); //cli.push(jr.data.qtd_dias_atr_dupl);
			cli.push(''); //cli.push(jr.data.qtd_dias_atr_med);
			log.info("passo 0005 5");
			cli.push(''); //cli.push(jr.data.val_ped_carteira);
			cli.push(''); //cli.push(jr.data.val_dup_aberto);
			cli.push(''); //cli.push(jr.data.dat_ult_fat);
			cli.push(''); //cli.push(jr.data.val_limite_cred);
			cli.push(''); //cli.push(jr.data.dat_val_lmt_cr);
			cli.push(''); //cli.push(jr.data.ies_nota_debito);
			...
			cli.push(wEndEnt);
			cli.push(wIndeLogradouroEnt);
			cli.push(wTipoLogradouroEnt);
			cli.push(wBairroEnt);
			cli.push(wComplEndEnt);
