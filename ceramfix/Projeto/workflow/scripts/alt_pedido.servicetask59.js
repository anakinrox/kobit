...
function servicetask59(attempt, message) {
	...
	hAPI.setCardValue("seqProcGer", getValueFormFloat("seqProcGer", "0")+1 );
	...
	if (dataset != null) {
		...
		pct_custo_financ = parseFloat( dataset.getValue(0, "pct_custo_financ") );
	...
	pct_custo_financ = isNull(pct_custo_financ,0);
	...
	if( peso_total == 0 || peso_total == undefined || peso_total == null || peso_total == ""  ){
		...
		while (contadorItem.hasNext()) {
			...
			if( seqItem != 0 && seqItem != undefined && campo == "COD_ITEM" ) {
				...
				var peso = getValueFormNumberFloat("peso___" + seqItem, "0");
	...
	var tipoFreteLogix = getValueForm('tipo_frete_logix','X');
	var ctTipo = new Array();
	ctTipo.push( DatasetFactory.createConstraint("tipo_frete_fluig", hAPI.getCardValue('tipo_frete'), hAPI.getCardValue('tipo_frete'), ConstraintType.MUST) );
	var dsTipo = DatasetFactory.getDataset('tipo_frete', ['tipo_frete_logix'], ctTipo, null);
	if( dsTipo.rowsCount > 0 ){
		tipoFreteLogix = dsTipo.getValue(0, "tipo_frete_logix");
		hAPI.setCardValue('tipo_frete_logix', tipoFreteLogix);
	}
	...
	while (contadorItem.hasNext()) {
		...
		if( seqItem != 0 && seqItem != undefined && campo == "COD_ITEM" ) {
			...
			var SQL = " select  nvl(EIS_F_GET_CUSTO_REAL_PAR('"+ codEmpresa +"', '"+ codItem +"', 'AD', 'S'),0) * "+ qtd +" as custo_aditivo," +
		       		...
		       		"			nvl(EIS_F_GET_CUSTO_REAL_PAR('"+ codEmpresa +"', '"+ codItem +"', 'MS', 'S'),0) * "+ pesoTotalItem +" as custo_material_secundario," +
       				...
       				"			nvl(EIS_F_GET_CUSTO_REAL_PAR('"+ codEmpresa +"', '"+ codItem +"', 'FT', 'S'),0) * "+ pesoTotalItem +" as custo_transferecia, "+
		       		...
		       		"			EIS_F_GET_PRZ_MEDIO( "+ codCondPgto +" ) as prazo_medio "+
		       		"	from dual ";
			...
			var prazo_medio = parseFloat(0);
			
			if (dataset != null) {
				 
				//hAPI.setCardValue("custo___" + seqItem, df2.format( parseFloat( dataset.getValue(0, "custo") ) ) );
				
				prazo_medio = parseFloat( dataset.getValue(0, "prazo_medio") );
				...
				var custo_finac = ( valor * pct_custo_financ / 100 ) / 30 * prazo_medio;
				
				log.info('frete...'+frete );
				lob_item = rol_item - custo - custo_finac - frete - desc_finac;
			...
			if (dataset != null) {
				...
				if (p_lob_item == null || p_lob_item == 'NaN') {
					log.info('entrou porque estava nulo');
					p_lob_item = 0;
				}
				...
				var verba = parseFloat( dataset.getValue(0, "verba_marketing") );
				var acordo = parseFloat( dataset.getValue(0, "acordo_comercial") )
							+ parseFloat( dataset.getValue(0, "verba_rapel") )
							+ parseFloat( dataset.getValue(0, "verba_cross_dock") );
				
				acordo_total += acordo;
				
				item["acordo_comerc"] = acordo;
	...
	hAPI.setCardValue("custo_finac", df2.format( custo_finac ) );
	
	log.info( 'variaveis LOB.........'+rol+' '+custo_total+' '+custo_finac+' '+comis_total+' '+acordo_total+' '+verba_total+' '+custo_trans_total+' '+frete_total+' '+desc_finac+' '+valor_adic_tipo_venda );
	
	lob = rol - custo_total - custo_finac - comis_total - acordo_total - verba_total - custo_trans_total - frete_total - desc_finac - valor_adic_tipo_venda;
	...
	//try
	{
		...
		log.info(' acumulado ');
		var SQL = " select NVL(SUM((EIS_V_COMERCIAL.VAL_LIQ_ITEM - "+
				...
				"	            EIS_V_COMERCIAL.VLR_COFINS_DEVOLUCAO_DEV)),0) rol, "+
				"	       NVL(SUM(EIS_V_COMERCIAL.custo_embalagem + "+
				...
				"	           EIS_V_COMERCIAL.custo_cimento),0) cpv, "+
				"	       NVL(SUM(EIS_V_COMERCIAL.VALOR_COMIS + "+
				...
				"	           EIS_V_COMERCIAL.frete_tabela),0) dvv, "+           
				"	       NVL(SUM(EIS_V_COMERCIAL.QTD_MIX -  "+
				"	           EIS_V_COMERCIAL.qtd_mix_dev),0) pesomix, "+
				"	       NVL(SUM(EIS_V_COMERCIAL.PESO_FATU -  "+
				"	           EIS_V_COMERCIAL.PESO_DECLARAD_NF_DEV),0) peso "+
	...
	try{
		...
		for( var p = 0; p < itens.length; p++ ){
			...
			if( itens[p] != undefined ){
			    ...
			    hAPI.setCardValue("perc_lob_item_controle___" + itens[p].seq_item, df2.format( itens[p].perc_lob_item ) );
			    ...
			    hAPI.setCardValue("ies_apv_desc_item_sreg___" + itens[p].seq_item, itens[p].ies_apv_desc_item_sreg );
	...
	if( ies_mix == 'on' ){
		...
		if ( getValueFormFloat( "mix_geral", "0")
			<= getValueFormFloat("perc_mix_ger_sreg", "0") ){
				ies_mix_geral_sreg = 'S';
		} 
		if ( getValueFormFloat("mix_geral", "0")
	...
	if( ies_desc_ped == 'on' ){
		...
		if ( getValueFormFloat("desc_medio", "0")
		    > ( getValueFormFloat("desc_maximo", "0")
		      + getValueFormFloat("perc_desc_ger_sreg", "0") ) ){
			ies_desc_medio_sreg = 'S';
		}
		if ( getValueFormFloat("desc_medio", "0")
			    > ( getValueFormFloat("desc_maximo", "0")
	...
	hAPI.setCardValue("ies_alt_list_sreg", ies_alt_list_sreg);
	...
	if (cli_exc_mix.trim() == 'S'){
		...
		hAPI.setCardValue("ies_mix_geral_sreg", "N" );
		hAPI.setCardValue("ies_pct_lob_reg", "N" );
		hAPI.setCardValue("ies_pct_lob_sreg", "N" );
	...
	try{
		...
		while(rsWD.next()){
			...
			log.info('Cheguei aqui ST...'+rsWD.getInt("seq_item")+ rsWD.getFloat("st"));
...
function getValueFormNumberFloat( campo, padrao ){
	log.info('getValueFormFloat.........'+campo+' '+padrao);
	var vnPadrao = padrao+"";
	var vnValor = getValueForm(campo, vnPadrao)+"";
	log.info('getValueFormFloat vnValor.........'+vnValor+' '+vnPadrao);
	var valor = parseFloat( vnValor );
	return isNull( valor, parseFloat( vnPadrao ) );
}
...
function loadRepres( cod_repres ){
	...
	try{
		...
		if ( dsRepCpl != null && dsRepCpl.rowsCount>0){
	    	for( var x = 0; x < dsRepCpl.rowsCount; x++ ){
	    		var ctValida = new Array();
	    		ctValida.push( DatasetFactory.createConstraint("documentid", dsRepCpl.getValue(x, "documentid"), dsRepCpl.getValue(x, "documentid"), ConstraintType.MUST) );
	    		ctValida.push( DatasetFactory.createConstraint("tipo_cadastro", "N", "N", ConstraintType.MUST_NOT) );
	    		var dsValida = DatasetFactory.getDataset("representante_compl", null, ctValida, null);
	    		if ( dsValida != null && dsValida.rowsCount > 0 ){
	    			log.info("## parametros ## "+ dsRepCpl.getValue(x, "perc_mix_ger_reg") );
	    			hAPI.setCardValue("perc_mix_ger_reg", dsRepCpl.getValue(x, "perc_mix_ger_reg") );
	    			hAPI.setCardValue("perc_mix_ger_nac", dsRepCpl.getValue(x, "perc_mix_ger_nac") );
	    			hAPI.setCardValue("perc_desc_ger_reg", dsRepCpl.getValue(x, "perc_desc_ger_reg") );
	    			hAPI.setCardValue("perc_desc_ger_nac", dsRepCpl.getValue(x, "perc_desc_ger_nac") );
	    			hAPI.setCardValue("perc_lob_ger_nac", dsRepCpl.getValue(x, "lob_ger_nac") );
	    			hAPI.setCardValue("perc_lob_ctr", dsRepCpl.getValue(x, "lob_controller") );  
	    			break;
		    	}
		    }    
	    }
		...
		var codRegional = "";
		if ( dsRep != null  ){
			codRegional = dsRep.getValue(0, "cv_5");
			log.info('codRegional......'+codRegional);
		}
		log.info("## Regional ## "+codRegional );
		var constraints = new Array();
		...
		if ( dsRepCpl != null && dsRepCpl.rowsCount>0){
	    	for( var x = 0; x < dsRepCpl.rowsCount; x++ ){
	    		...
	    		var dsValida = DatasetFactory.getDataset("representante_compl", null, ctValida, null);
	    		if ( dsValida != null && dsValida.rowsCount > 0 ){
	    			log.info("## parametros ## "+ dsRepCpl.getValue(x, "perc_mix_ger_sup_reg") );
	    			hAPI.setCardValue("perc_mix_ger_sreg", dsRepCpl.getValue(x, "perc_mix_sup_reg") );
	    			hAPI.setCardValue("perc_desc_ger_sreg", dsRepCpl.getValue(x, "perc_desc_sup_reg") ); 
	    			break;
		    	}
		    }    
	    }
		...
		if ( dsRepCpl != null && dsRepCpl.rowsCount>0){
			...
			for( var x = 0; x < dsRepCpl.rowsCount; x++ ){
		    	...
		    	if ( dsValida != null && dsValida.rowsCount > 0 ){
				    ...
				    if( dsRepCpl.getValue(x, "cod_supervisor") != ""
					 && dsRepCpl.getValue(x, "cod_supervisor") != undefined ){
						
						hAPI.setCardValue("ies_supervisor", "S");
					
						var idPai = dsRepCpl.getValue(x, "metadata#id"); 
						var versionPai = dsRepCpl.getValue(x, "metadata#version");
