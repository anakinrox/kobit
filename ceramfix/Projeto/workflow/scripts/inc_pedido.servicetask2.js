function servicetask2(attempt, message) {
	...
	var codEmpresa = getValueInt("empresa", "");
	var itemSolicAcum = {};
	var dataHora = new Date();

	var numProcesso = getValueInt('WKNumProces');
	var userId = getValueInt("WKUser");
	...
	var camposItem = hAPI.getCardData(getValueInt("WKNumProces"));
	...
	if (informa_aen_pedido == "S") {
		log.info('Evento Processo ..... 001 A...'
			+ getValueInt("cod_repres", "0") + '...');
		var c1 = DatasetFactory.createConstraint("cod_repres", getValueInt(
			"cod_repres", "0"), getValueInt("cod_repres", "0"),
			ConstraintType.MUST);
	...
	while (contadorItem.hasNext()) {
		...
		if (seqItem != undefined && campo == "COD_ITEM") {
			...
			var codItem = getValueInt("cod_item___" + seqItem, "") + "";
			...
			var qtd = Math.round(getValueFloat("quantidade___" + seqItem, 0) * 1000) / 1000;
			if (qtd == null) {
				qtd = 0;
			}
			...
			dataEnt = format
				.parse(getValueInt("data_entrega___" + seqItem, "").split(
					"-").reverse().join("/"));
			...
			log.info("Read "
				+ getValueInt("data_entrega_calc___" + seqItem, ""));
		}
	}
	...
	var data = {
		companyId: getValueInt("WKCompany") + "",
	...
	var lr_principal = {};
	lr_principal["cod_empresa"] = getValueInt("empresa", "") + "";
	lr_principal["num_pedido"] = 0;
	var cod_cliente = getValueInt("cod_cliente", "");
	lr_principal["cod_cliente"] = cod_cliente + "";
	lr_principal["dat_emis_repres"] = getValueInt("data_emissao", "").split(
		"/").reverse().join("-");
	lr_principal["cod_nat_oper"] = getValueInt("nat_operacao", "") + "";
	...
	if (getValueInt("ins_estadual", "") == ""
		|| getValueInt("ins_estadual", "") == null
		|| getValueInt("ins_estadual", "") == undefined) {
	...
	var insEstadual = getValueInt("ins_estadual", "").replace("[^0-9]", "").trim();
	var iesTipFornec = getValueInt("ies_tip_fornec", "").trim();
	log.info("## INSCRICAO ## " + insEstadual);
	log.info("## COD_CLASS ## " + getValueInt("cod_class", "A"));
	var finalidade = "1";
	...
	} else if ((getValueInt("cod_class", "A") != "U" || getValueInt("cod_class", "A") != "u")
		&& (insEstadual == "" || insEstadual == null || insEstadual == "ISENTO")
		&& ((getValueInt("eh_contribuinte", "A")) == 'S')) {
		finalidade = "1";
	...
	} else if ((getValueInt("cod_class", "A")) == "U" && insEstadual != "" && insEstadual != null && insEstadual != "ISENTO" && (getValueInt("eh_contribuinte", "A")) == 'N') {
		finalidade = '2';
		log.info('## FINALIDADE ## 0004');
	} else if ((getValueInt("cod_class", "A") == "U" || getValueInt(
		"cod_class", "A") == "u")
		&& (insEstadual != "" || insEstadual != null || insEstadual != "ISENTO")) {
		finalidade = "3";
	...
	lr_principal["cod_cnd_pgto"] = getValueInt("cond_pagamento", "0") + "";
	lr_principal["ies_tip_entrega"] = getValueInt("tipo_entrega", "0") + "";
	lr_principal["cod_tip_venda"] = getValueInt("tipo_venda", "1") + "";
	lr_principal["cod_tip_carteira"] = getValueInt("cod_tip_carteira_cli",
		"01")
		+ "";
	...
	lr_representante["ies_comissao"] = getValueInt("ies_emite_dupl", "S") + "";
	lr_representante["cod_repres"] = getValueInt("cod_repres", "0") + "";
	...
	var lr_adicionais = {};
	lr_adicionais["num_pedido_repres"] = getValueInt("WKNumProces") + "";
	lr_adicionais["num_pedido_cli"] = getValueInt("ped_cliente", "") + "";
	...
	if (getValueInt("tipo_frete", "") == "RET") {
		lr_frete["cod_transpor"] = getValueInt("cod_cliente", "") + "";
	} else {
		lr_frete["cod_transpor"] = getValueInt("cod_trans", "") + "";
	}
	lr_frete["cod_consig"] = "";
	log.info(' informação de tipo de frete no formulário..... '
		+ getValueInt("tipo_frete_logix", "x"));
	lr_frete["ies_frete"] = getValueInt("tipo_frete_logix", "1") + "";
	...
	lr_preco_desconto["pct_desc_financ"] = getValueFloat("pct_desc_financ", "0") + "";
	lr_preco_desconto["pct_desc_adic"] = getValueFloat("pct_desc_adicional", "0") + "";
	lr_preco_desconto["num_list_preco"] = getValueInt("lista_preco", "") + "";
	lr_preco_desconto["cod_moeda"] = getValueInt("moeda", "") + "";
	if (getValueInt("tipo_desconto", "") == "") {
		lr_preco_desconto["tip_desc"] = null;
	} else {
		lr_preco_desconto["tip_desc"] = getValueInt("tipo_desconto", "") + "";
	}
	...
	if (hAPI.getCardValue("moeda") != "1") {
		...
		if (hAPI.getCardValue("cotacao").replace('.', '').replace(',', '.') == null ||
			...
			log.info('entrou no ajuste da cotacao da moeda porque veio sem nada 2 !!! ' + getValueInt("moeda", "") + " " + getValueInt("data_emissao", ""));
			var c1 = DatasetFactory.createConstraint("cod_moeda", getValueInt("moeda", ""), getValueInt("moeda", ""), ConstraintType.MUST);
			var c2 = DatasetFactory.createConstraint("dat_ref", getValueInt("data_emissao", ""), getValueInt("data_emissao", ""), ConstraintType.MUST);
	...
	if (hAPI.getCardValue("moeda") != "1") {
		...
		lr_preco_desconto["val_cotacao_fixa"] = getValueFloat("cotacao", "1") + "";
	...
	if (getValueInt("endereco_ent", "") != "") {
		lr_entrega["end_entrega"] = getValueInt("tipo_logradouro_ent", "")
			+ " " + getValueInt("endereco_ent", "") + ", "
			+ getValueInt("numero_ent", "") + "";
		lr_entrega["den_bairro"] = getValueInt("bairro_ent", getValueInt(
			"bairro_ent_sel", ""))
			+ "";
		lr_entrega["cod_cidade"] = getValueInt("cod_cidade_ent", "") + "";
		lr_entrega["cod_cep"] = getValueInt("cep_ent", "") + "";
		lr_entrega["num_cgc"] = getValueInt("cnpj_ent", "") + "";
		lr_entrega["ins_estadual"] = getValueInt("ie_ent", "") + "";
		lr_entrega["nom_cliente_end_ent"] = getValueInt("razao_social", "")
			+ "";
	} else {
	...
	lr_textos_pedido["tex_observ_1"] = getValueInt("texto_pedido_1", "") + "";
	lr_textos_pedido["tex_observ_2"] = getValueInt("texto_pedido_2", "") + "";
	lr_textos_pedido["den_texto_1"] = getValueInt("texto_nf_1", "") + "";
	lr_textos_pedido["den_texto_2"] = getValueInt("texto_nf_2", "") + "";
	lr_textos_pedido["den_texto_3"] = getValueInt("texto_nf_3", "") + "";
	lr_textos_pedido["den_texto_4"] = getValueInt("texto_nf_4", "") + "";
	lr_textos_pedido["den_texto_5"] = getValueInt("texto_nf_5", "") + "";
	...
	while (contadorItem.hasNext()) {
		...
		if (seqItem != undefined && campo == "COD_ITEM") {
			...
			la_pedido_itens["cod_item"] = getValueInt("cod_item___" + seqItem, "") + "";
			la_pedido_itens["pct_desc_adic"] = getValueFloat("desconto___" + seqItem, "0") + "";
			la_pedido_itens["pre_unit"] = getValueFloat("valor_unit___" + seqItem, "0") + "";
			la_pedido_itens["qtd_pecas_solic"] = getValueFloat("quantidade___" + seqItem, "0") + "";

			var dataEntrega = format.parse(getValueInt("data_entrega___" + seqItem, "").split("-").reverse().join("/"));
			var dataEntregaCal = format.parse(getValueInt("data_entrega_calc___" + seqItem, ""));

			if (getValueInt("tipo_entrega", "0") == '1') {
			...
			la_pedido_itens["den_texto_1"] = getValueInt("obs_item_1___" + seqItem, "") + "";
			la_pedido_itens["den_texto_2"] = getValueInt("obs_item_2___" + seqItem, "") + "";
			la_pedido_itens["den_texto_3"] = getValueInt("obs_item_3___" + seqItem, "") + "";
			la_pedido_itens["den_texto_4"] = getValueInt("obs_item_4___" + seqItem, "") + "";
			la_pedido_itens["den_texto_5"] = getValueInt("obs_item_5___" + seqItem, "") + "";
			la_pedido_itens["xped"] = getValueInt("pedido_cli_item___" + seqItem, "") + "";
			la_pedido_itens["nitemped"] = getValueInt("seq_pedido_cli_item___" + seqItem, "") + "";
	...
	try {
		...
		constraints.push(DatasetFactory.createConstraint('cod_empresa', getValueInt("empresa", ""), getValueInt("empresa", ""), ConstraintType.MUST));
		constraints.push(DatasetFactory.createConstraint('num_pedido_repres', numProcesso, numProcesso, ConstraintType.MUST));
		constraints.push(DatasetFactory.createConstraint('dat_emis_repres', getValueInt("data_emissao", ""), getValueInt("data_emissao", ""), ConstraintType.MUST));
...
function calcDataEntrega(codEmpresa, codItem, dataHora, qtd_acum) {
	...
	try {
		...
		//statementWD.setDate(3, sdf.format(new java.sql.Date(data)));
		log.info('PP 00004.........  ');
