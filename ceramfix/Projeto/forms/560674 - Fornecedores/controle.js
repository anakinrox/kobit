...
function validaAba( aba ){
	...
	/*if ( aba == 1 && $('#tipo_cadastro').val() == 'N' && validaExistCNPJ( $('#cnpj').val() ) && ($('#task').val() != 4)){
		...
		if( $('#cnpj').val().indexOf('/0000-') == -1  ){
			msgTipo = 'CNPJ ja cadastrado, apenas para homologação!';
		}else{
			msgTipo = 'CPF ja cadastrado, apenas para homologação!';
		}
...
//	$( '#valEditRepres' ).val( 'N' );
	...
	$("input").keyup(function(){
		$("input").keyup(function(){
			if( $(this).attr("id") == "compra_estimada" ) return true;
			$(this).val(
...
function setHomologacao(){
	if ( $('#homologacao').val() == 'S'  ){
		$('.divHomolog').show();
	}else {
		$('.divHomolog').hide();
	}
...
function loadCompraMinima(){
	

	var constraintsFilhos = new Array();
	constraintsFilhos.push( DatasetFactory.createConstraint("dataset", 'empresa_compl', 'empresa_compl', ConstraintType.MUST) );
	constraintsFilhos.push( DatasetFactory.createConstraint("p.cod_empresa", '99', '99', ConstraintType.MUST) );
	var datasetFilhos = DatasetFactory.getDataset('selectDataset', ['p.valor_aprovacao_minima'], constraintsFilhos, null);
	var filhos = datasetFilhos.values;
	for (var i = 0; i < filhos.length; i++){
		$("#compra_minima").val(datasetFilhos.values[0]['valor_aprovacao_minima']);
	}
}

function setCompraMinima(){
	var compra_estimada = 0;
	if ( $('#compra_estimada').val() != '' ) compra_estimada = parseFloat( $('#compra_estimada').val().replaceAll('.','').replace(',','.') );
	if( compra_estimada >  parseFloat( $('#compra_minima').val().replace(',','.')) ){
		$('#ies_compra_minima').val('S');
	} else {
		$('#ies_compra_minima').val('N');
	}
