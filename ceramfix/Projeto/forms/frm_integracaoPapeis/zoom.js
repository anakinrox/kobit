function openZoom({ field, filters }) {
    ...
    const configs = {
        ...
        papel: ['Papeis', 'workflowRole', 'roleId,Código,roleDescription,Grupo', 'roleId,roleDescription', filtersString, field, false, 'default', null, null, 'roleDescription']
...
function setSelectedZoomItem({ type, cargoId, cargoNome, estruturaId, estruturaNome, alias, name, lojaCodigo, lojaNome, GROUP_CODE, DESCRIPTION, roleId, roleDescription, ...item }) {
    ...
    if (type === 'papel') {
        document.getElementById('cod_papel').value = roleId;
        document.getElementById('nome_papel').value = roleDescription;
        return;
    }
}
...
function clearSelectedZoomItem(type) {
    ...
    if (type === 'papel') {
        document.getElementById('cod_papel').value = '';
