...
function setSelectedZoomItem(selectedItem) {
	...
	if (selectedItem.inputName == 'representante') {
		...
		$('#carteira').val(selectedItem.carteira);
		$('#cod_carteira').val(selectedItem.cod_carteira != '' ? selectedItem.cod_carteira : "99");
	}
...
function loadCliente(codCliente) {
	...
	var constraints = new Array(c1);
	// Busca o dataset
	var dataset = DatasetFactory.getDataset('cliente_rest2', null, constraints, null);
	if (dataset == null) {
		FLUIGC.toast({
			title: 'Conexao: ',
	...
	for (var x = 0; x < dataset.values.length; x++) {
		...
		$('#rede_').val(row['num_telex']);
		$('#rede_aux').val(row['num_telex']);



		$('#complemento').val(row['compl_endereco']);
		if (row['email_cob'] != '') {
			var index = addEmailColab();
			if (index != -1) {
				$('#e_mail_cob___' + index).val(row['email_cob']);
		...
		$('#qtd_dias_atr_dupl').val(row['qtd_dias_atr_dupl'] != '' ? row['qtd_dias_atr_dupl'] : '');
		$('#qtd_dias_atr_med').val(row['qtd_dias_atr_med'] != '' ? row['qtd_dias_atr_med'] : '');
		$('#val_ped_carteira').val(row['val_ped_carteira'] != '' ? row['val_ped_carteira'] : '');
		$('#val_dup_aberto').val(row['val_dup_aberto'] != '' ? row['val_dup_aberto'] : '');
		$('#dat_ult_fat').val(row['dat_ult_fat'] != '' ? row['dat_ult_fat'] : '');
		$('#val_limite_cred').val(row['val_limite_cred'] != '' ? row['val_limite_cred'] : '');
		$('#dat_val_lmt_cr').val(row['dat_val_lmt_cr'] != '' ? row['dat_val_lmt_cr'] : '');
