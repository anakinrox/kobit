...
$(document).bind("DOMNodeRemoved", function(e){
	...
	if ( e.target.nodeName == 'LI' ){	
		...
		if( target.html().indexOf('class="select2-selection__choice"' ) > 0 ){
			...
			if( aba == 8 && $('#representante').val() == null 
...
function validaAba( aba ){
		  ...
		  || ( $(this).attr( 'pf' ) == "S" && $(this).attr( 'name' ).indexOf('___') != -1 ) )
			...
			if ( $(this).attr( 'valida' ) != undefined  
				...
				if (!( ( aba == 1 && $('#tipo_cadastro' ).val() == 'A' && $(this).attr( 'id' ) == 'razao_social' ) ||
					 ...
					 ( aba == 1 && $('#cod_pais').val() != '001' && $(this).attr( 'id' ) == 'cep' ) ||
					 ( aba == 1 && $('#cod_pais').val() != '001' && $(this).attr( 'id' ) == 'cnpj' ) /*||
	...
	if ( aba == 8 && $('#situacao_cadastro').val() == 'R' && $('#motivo_reprovacao').val() == '' ){
		FLUIGC.toast({
				title: 'Validacao: ',
	...
	if ( aba == 1 && $('#tipo_cadastro').val() == 'N' && !$('#internacional').is(':checked') && validaExistCNPJ( $('#cnpj').val() ) && ($('#task').val() != 4)){
...
function setInter(){
	if ( $('#internacional').is(':checked') ){
		$('.divCnpjCpf').hide();
		$('.divRUC').show();
		$('#cnpj').removeAttr('valida');
		$('#cnpj').removeAttr('regra');
		$('#ruc').attr('valida','RUC');
		$('#cnpj').val( "000.000.000/0000-00" );
	} else {
		$('.divCnpjCpf').show();
		$('.divRUC').hide();
		$('#cnpj').attr('valida','CNPJ / CPF');
		$('#cnpj').attr('regra','cnpjcpf');
		$('#ruc').removeAttr('valida');
		if( $('#cnpj').val() == "000.000.000/0000-00" ){
			$('#cnpj').val( "" );
		}
	}
}
...
async function translate(){
	...
	// labels
	$('label').each( (i ,el) => {
		var texto = el.innerText.replaceAll('\t','').replaceAll('\n','').trim();
		if (texto != ''){
			var obj = retira_acentos( texto.toLowerCase() ).replaceAll(' ','_');
			wJson[obj] = texto
		}
	});
	...
	// tables
	$('table').each( (i ,el) => {
		if ( el.firstElementChild.tagName == 'THEAD'){        
			var array = el.firstElementChild.innerText.replaceAll('\n','').split('\t');
			var filteredArray = array.filter(index => index != '')
			for (var item in filteredArray){
				var obj = retira_acentos( filteredArray[item].toLowerCase() ).replaceAll(' ','_')
				wJson[obj] = filteredArray[item];
			}
		}
	});
	...
	$('.panel-heading').each( (i ,el) => {
		var obj = retira_acentos( el.innerText.trim().toLowerCase() ).replaceAll(' ','_');
		wJson[obj] = el.innerText.trim()
	});

	for ( var item in wJson ){
		var sourceText = wJson[item];
		translate_pt += item + "="+ sourceText + "\n";
		
		var requestOptions = {
			method: 'GET',
			redirect: 'follow'
		};		  
		const response_en = await fetch("https://translate.googleapis.com/translate_a/single?client=gtx&sl=pt&tl=en&dt=t&q=" + encodeURI(sourceText), requestOptions)
		const text_en = await response_en.json();
		// console.log( item , text_en[0][0][0] );
		translate_en += item + "="+ text_en[0][0][0] + "\n";

		const response_es = await fetch("https://translate.googleapis.com/translate_a/single?client=gtx&sl=pt&tl=es&dt=t&q=" + encodeURI(sourceText), requestOptions)
		const text_es = await response_es.json();
		// console.log( item , text_es[0][0][0] );
		translate_es += item + "="+ text_en[0][0][0] + "\n";
	}
	...
	link.href = "data:text/plain;charset=utf-8," + encodeURIComponent(stringData);
	link.click();
	*/
...
function addEmailColab() {
	var idAtual = ''
	$("input[id^='fId___']").each(function() {
		var index  = $(this).prop('id').replace('fId___', '');
		idAtual = $('#fId___' + index).val();
	})
	if(idAtual < 3){
		wdkAddChild('tbEmailBoleto');
		$('#fId___' + newId).val(newId);
		$('#qtdEmails').val(parseInt( $('#qtdEmails').val())+1);
	}else{
		FLUIGC.toast({
			title: 'Atenção: ',
			message: 'Não pode adicionar mais de 3 emails para boleto! ',
			type: 'warning',
			timeout: 'slow'
		});	
		return -1;
...
function verificaAcessoRedes() {
	
	//var user = parent.WCMAPI.userCode;
	var user = $("#userAtual").val();
	
	var c1 = DatasetFactory.createConstraint('workflowColleagueRolePK.roleId', 'lider-comercial', 'lider-comercial', ConstraintType.MUST);
	var c2 = DatasetFactory.createConstraint('workflowColleagueRolePK.colleagueId', user, user, ConstraintType.MUST);
	var ds_admVendas = DatasetFactory.getDataset('workflowColleagueRole', null, [c1, c2], null);

	if(ds_admVendas.values.length > 0 && $("#temAcessoRedes").val() == 'nao'){
		$("#div_rede").show();
		$("#temAcessoRedes").val('sim')
	}else{
		//var user = parent.WCMAPI.userCode;
		var user = $("#userAtual").val();
		var c1 = DatasetFactory.createConstraint('workflowColleagueRolePK.roleId', 'crc', 'crc', ConstraintType.MUST);
		var c2 = DatasetFactory.createConstraint('workflowColleagueRolePK.colleagueId', user, user, ConstraintType.MUST);
		var ds_crc = DatasetFactory.getDataset('workflowColleagueRole', null, [c1, c2], null);	

		if(ds_crc.values.length > 0){
			$("#div_rede").show();
			$("#temAcessoRedes").val('sim')
		}else{
			$("#div_rede").hide();
