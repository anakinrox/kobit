	  ...
	  //return false;
			...
			function zoomButton(componente) {
				...
				if ( componente == 'btCliente' ){
					...
					if( $('#tipoCadUser').val() == "V" ) {
						modalzoom.open("Clientes",
									   "clientes_logix", 
									   "cod_cliente,Cod. Cliente,nom_cliente,Nome,cnpj,CNPJ,den_cidade,Cidade,cod_uni_feder,UF,cod_tip_carteira,Carteira", 
									   "cod_cliente,nom_cliente,cnpj,cod_repres,cod_user,den_cidade,cod_uni_feder,cod_tip_carteira", 
									   "COD_USER_SUPERVISOR,"+$('#userFluig').val(),
									   componente, false, "default", null, null,
									   "cod_cliente||'-'||nom_cliente||'-'||cnpj||'-'||den_cidade||'-'||cod_uni_feder" );	
					} else if( $('#tipoCadUser').val() == "G" ) {
						modalzoom.open("Clientes",
								   "clientes_logix", 
								   "cod_cliente,Cod. Cliente,nom_cliente,Nome,cnpj,CNPJ,den_cidade,Cidade,cod_uni_feder,UF,cod_tip_carteira,Carteira", 
								   "cod_cliente,nom_cliente,cnpj,cod_repres,cod_user,den_cidade,cod_uni_feder,cod_tip_carteira", 
								   "COD_USER_GERENTE,"+$('#userFluig').val(),
								   componente, false, "default", null, null,
								   "cod_cliente||'-'||nom_cliente||'-'||cnpj||'-'||den_cidade||'-'||cod_uni_feder" );	
					} else if ( $('#tipoCadUser').val() == "A" ) {
						modalzoom.open("Clientes",
									   "clientes_logix", 
									   "cod_cliente,Cod. Cliente,nom_cliente,Nome,cnpj,CNPJ,den_cidade,Cidade,cod_uni_feder,UF,cod_tip_carteira,Carteira", 
									   "cod_cliente,nom_cliente,cnpj,cod_repres,cod_user,den_cidade,cod_uni_feder,cod_tip_carteira", 
				...
				if ( componente == 'btPedido' ){
					...
					if( $('#tipoCadUser').val() == "A" || $('#tipoCadUser').val() == "G" || $('#tipoCadUser').val() == "V" ) {
						...
						if( $('#tipoCadUser').val() == "A" ){
						...
						}else if( $('#tipoCadUser').val() == "V" ){
							
							var newConstraints = [];
							newConstraints.push( DatasetFactory.createConstraint("nivel", 7, 7, ConstraintType.MUST) );
						    newConstraints.push( DatasetFactory.createConstraint("cod_nivel_5", $('#userFluig').val(), $('#userFluig').val(), ConstraintType.MUST) ); 
							var dataset = DatasetFactory.getDataset("canal_vendas", null, newConstraints, sortingFields);
							if ( dataset != null ){
								for (var x = 0; x < dataset.values.length; x++) {
									codRepres += '|'+dataset.values[x].cod_nivel_7 ;
									console.log('Repres....',codRepres );
								}
							}
							...
							var newConstraints = [];
							newConstraints.push( DatasetFactory.createConstraint("nivel", 7, 7, ConstraintType.MUST) );
						    newConstraints.push( DatasetFactory.createConstraint("cod_nivel_4", $('#userFluig').val(), $('#userFluig').val(), ConstraintType.MUST) ); 
							var dataset = DatasetFactory.getDataset("canal_vendas", null, newConstraints, sortingFields);
							if ( dataset != null ){
								for (var x = 0; x < dataset.values.length; x++) {
									codRepres += '|'+dataset.values[x].cod_nivel_7 ;
						...
						modalzoom2.open("Pedidos",
								       ...
								       "cod_empresa,den_reduz,num_cgc_cpf,cod_cliente,nom_cliente,nom_reduzido,num_pedido,dat_pedido,qtd_saldo,val_pedido,cod_repres,cod_nat_oper,tip_frete,cod_tip_carteira", 
						...
						modalzoom2.open("Pedidos",
									   "selectTable", 
									   "den_reduz,Emp,num_pedido,Pedido,dat_pedido,Data,nom_cliente,Cliente,val_pedido,Valor", 
									   "cod_empresa,den_reduz,cod_cliente,num_cgc_cpf,nom_cliente,nom_reduzido,num_pedido,dat_pedido,qtd_saldo,val_pedido,cod_repres,cod_nat_oper,tip_frete,cod_tip_carteira", 
									   "dataBase,java:/jdbc/LogixPRD,table,eis_v_fluig_ped_aberto,ies_alter_ped,S,sqlLimit,10,___in___cod_repres,"+codRepres+",___in___cod_nat_oper,"+codNat+",___not______in___empresa_lista,"+listaEsp,
									   componente, true, "default", null, null,
									   "den_reduz||'-'||num_pedido||'-'||dat_pedido||'-'||nom_reduzido||'-'||nom_cliente||'-'||cod_cliente" );				
						}
					...
					if (componente == 'bt_repres') {
						...
						return true;
						//if( $('#tipoCadUserAtu').val() == 'R' ) return true;
			...
			function setSelectedZoomItem(selectedItem) {              
				...
				if( selectedItem.type == 'btCliente' ){
					...
					loadCliente( selectedItem.cod_cliente,selectedItem.cod_tip_carteira );
				...
				if (selectedItem.type == "bt_repres") {
					   $('#cod_carteira').val( selectedItem.cod_cart ) ;
				...
				if( selectedItem.inputName == 'razao_social' ){
					...
					FLUIGC.message.confirm({
					    message: "Deseja carregar capa do pedido anterior?",
					    labelYes: 'Confirmar',
					    labelNo: 'Cancelar'
...
/*			function loadDataSetCombo( combo, dataSet, fieldCodigo, fieldDesc, fildFilter, fildFilterValue ){
			...
			function loadCliente( codCliente,codCarteira ){
				
				// We can show the message of loading
				console.log("t Antes do load..... "+codCliente);
				
				//alert("cliente...."+codCliente);
				
				loadingPrincipial.show();

				var constraints = new Array();
				constraints.push( DatasetFactory.createConstraint("cod_cliente", codCliente, codCliente, ConstraintType.MUST) );
				constraints.push(DatasetFactory.createConstraint("cod_tip_carteira", codCarteira,
						codCarteira, ConstraintType.MUST));
						...
						if ( info == 'TV' || info == 'TT' ){
							...
							if ($('#tipoCadUser').val() == "R" ) {
								...
								if ($('#tipoCadUser').val() == "A" ){
									...
									var repCompl = DatasetFactory.getDataset('representante_compl', null, cons, null);	
									if ( repCompl != null && repCompl != undefined ){
										//var selectedItem = datasetPai.values[0];
						            	//console.log( selectedItem );
										console.log('REPRESENTANTE ', repCompl.values[0].cod_repres);
										var cod_repres_user = repCompl.values[0].cod_repres;
...
function buscaUltimoPreco( id ){
	...
	if ( datasetCliItem != null && datasetCliItem.values.length ){
		...
		getDescModal(  datasetCliItem.values[0]['PRE_UNIT'], datasetCliItem.values[0]['PCT_DESC_ADIC'], datasetCliItem.values[0]['ULT_PRECO_UNIT_LIQ'], datasetCliItem.values[0]['DATA_ULT_PRECO']  );		
	...
	if ( pedido != null ){
		for (var x = 0; x < pedido.values.length; x++) {						
			...
			if ( datasetUlt != null && datasetUlt.values.length > 0 ){
				...
				$('#ult_dat_preco_unit_liq___'+seq).val( regUlt.DATA_ULT_PRECO );
			} else {
				console.log("zeros");
				$('#ult_preco_unit_liq___'+seq).val( '0,00' );
				$('#ult_dat_preco_unit_liq___'+seq).val( '' );
			}
