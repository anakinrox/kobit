...
function nextPage() {
	...
	if( $('#processo').val() == "alt_pedido" && $('#num_pedido').val() == "" ) {
		alert("Você deve selecionar um pedido para um processo de alteração.");
		return false;
	}
...
function trataGer(){
	...
	if ( $("#tipoCadUserAtu").val() == 'R' && $("#visualiza_lob").val() != "S" ){
		$(".linha_gerencial").hide();
	}else{
...
function loadBody() {
	...
	//var dataset = DatasetFactory.getDataset('empresa', null, null, null);
	...
	if ($('#processo').val() == "inc_pedido") {
		if (!($('#task').val() == "0" || $('#task').val() == "1" || $('#task').val() == "33")) {
			$('.edit_item').hide();
	...
	if ($('#ped_representante').val != "0") {
		...
		loadRepresCompl($('#cod_repres').val(), $('#cod_cliente').val(), $('#cod_moeda_cli').val(), 'TT');
...
function setFilterCliente(qtd) {
	...
	try {
		// loading.show();
		if ($('#tipoCadUser').val() == 'A' || $('#tipoCadUser').val() == 'N') {
			console.log('TIPO_CADASTRO_DE_USUARIO2', $('#tipoCadUser').val());
			reloadZoomFilterValues('razao_social', "");
		} else if ($('#tipoCadUser').val() == 'V') {
			console.log('TIPO_CADASTRO_DE_USUARIO2', $('#tipoCadUser').val());
			reloadZoomFilterValues("razao_social","COD_USER_SUPERVISOR,"+$("#userFluig").val() );
		} else if ($('#tipoCadUser').val() == 'S') {
			console.log('TIPO_CADASTRO_DE_USUARIO2', $('#tipoCadUser').val());
			reloadZoomFilterValues("razao_social","COD_USER_SUPERVISOR,"+$("#userFluig").val() );
		} else if ($('#tipoCadUser').val() == 'G') {
			console.log('TIPO_CADASTRO_DE_USUARIO2', $('#tipoCadUser').val());
			reloadZoomFilterValues("razao_social","COD_USER_GERENTE,"+$("#userFluig").val() );
		} else {
...
function gravaItem(table, alteraMotivo) {
	...
	$('#ult_dat_preco_unit_liq___' + seq).val($('#ult_dat_preco_unit_liq_edit').val());
	$('#ult_preco_unit_liq___' + seq).val($('#ult_preco_unit_liq_edit').val());
...
function getDescModal(preco, desc, ultPreco, ultData) {
	...
	html += " Desconto: " + String((desc).toFixed(2)).replace('.', ',')
			+ "% <br>";
	html += " Ultimo Preço: R$ " + String((ultPreco).toFixed(2)).replace('.', ',')
			+ " <br>";
	html += " Ultima Data: " + ultData.split("-").reverse().join("/")
			+ " </div>";
		...
		if (err) {
			...
			$('.confirmar').click(
					function() {
						...
						$('#ult_dat_preco_unit_liq_edit').val( ultData.split("-").reverse().join("/") );
						$('#ult_preco_unit_liq_edit').val( String((ultPreco).toFixed(2)).replace('.', ',') );
