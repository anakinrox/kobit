    ...
    try {
        ...
        if (vo.getResult() == "" || vo.getResult().isEmpty()) {
            ...
            if (jr.STATUS == 'OK') {
                ...
                // f_postTipoVisita(wToken, newDataset);
...
function f_postTipoVisita(wToken, pDataset) {
    // ################### Envia os Locais #########################
    var params;
    ...
    if (dataset != null && dataset.rowsCount > 0) {
        ...
        if (tpTipos != '') {
            ...
            printLog('info', 'JSON Tipo: ' + gson.toJson(tpTipos))
            // var locais = f_atualizarRegistro(endpoint, wToken, tpTipos)
            // if (locais != null) {
            //     if (locais.STATUS == 'OK') {
            //         pDataset.addRow(new Array("Tipos de visitas Enviados"));
            //     } else {
            //         pDataset.addRow(new Array("Erro ao cadastrar Tipos de visitas"));
            //     }
            // }
        }
    }
...
function f_postInstalacoes(wToken, pDataset) {
    ...
    printLog("info", "End Point Instaladores: ");
    ...
    sql += "where kbt.indsituacao IN ('P','D') ";
    ...
    while (rsWD.next()) {
        ...
        var dados = {
            INDACAO: rsWD.getString("indsituacao") + "",
        ...
        if (arrInstaladores.length > 0) {
            ...
            if (instalacao != null) {
                if (instalacao.STATUS == 'OK') {
                    ...
                    if (rsWD.getString("indsituacao") == 'P') {
                        SQLINS = "update kbt_t_instalacoes set ind_sync = null, indsituacao = 'E' where id = " + rsWD.getString("id");
                    }
                    if (rsWD.getString("indsituacao") == 'D') {
                        SQLINS = "delete kbt_t_instalacoes_itens where idproposta = " + rsWD.getString("idProposta");
                        var statementWD = connectionWD.prepareStatement(SQLINS);
                        statementWD.executeUpdate();

                        SQLINS = "delete kbt_t_instalacoes where id = " + rsWD.getString("id");
                    }
    ...
    if (eventos != null) {
        for (var x = 0; x < eventos.length; x++) {
            if (eventos[x].STATUS == 'OK') {
                // if (eventos[x].ID == 2263) {
                var idCard = f_incluirCRM(eventos[x], newDataset);
                // newDataset.addRow(new Array('ID CARD: ' + idCard));
                var arrParceiros = [];
                var dados = {
                    INDACAO: "L",
                    ID: eventos[x].ID,
...
function f_getPipe(wToken, pDataset) {
    ...
    var dataDias = dataAtualFormatada(10, '-');
    ...
    if (leads != null) {
        for (var x = 0; x < leads.length; x++) {
            if (leads[x].STATUS == 'OK') {
                ...
                if (leads[x].CPFMOTORISTA != '70283621915') {
                    continue;
                }

                var wSQLUPD
                pDataset.addRow(new Array("Motorista: " + leads[x].CPFMOTORISTA + ' Contato: ' + f_removeCaracteres(leads[x].CONTATO)));
                try {
                    var wHora = null;
                    var wAchou = false;
                    var sql = '';
                    ...
                    if (leads[x].ORIGEM == 'PM') {
                        ...
                        wSQLUPD = "update kbt_t_registroapp set codtipovisita = " + leads[x].CODTIPOVISITA + ", tipovisita = '" + leads[x].TIPOVISITA + "' where origen = 'PM' and codvisita = " + leads[x].CODVISITA;
                    ...
                    if (leads[x].ORIGEM == 'EV') {
                        ...
                        wSQL += "((select COALESCE(max(sequencia) + 1,1) from kbt_t_registroapp)," + leads[x].ID + ",'" + leads[x].ORIGEM + "','" + leads[x].CPFMOTORISTA + "','" + f_removeCaracteres(leads[x].MOTORISTA) + "',";
                        wSQL += + leads[x].DOCUMENCTID + "," + leads[x].CODEVENTO + ",'" + leads[x].EVENTO + "','" + f_removeCaracteres(leads[x].CONTATO) + "','" + leads[x].DATA.split('/').reverse().join('-') + "'," + wHora + ",";
                    ...
                    if (wAchou == false) {
                        ...
                        // if (wSQLUPD != '') {
                        //     statementWD = connectionWD.prepareStatement(wSQLUPD);
                        //     statementWD.executeUpdate();
                        // }
                    }
                    ...
                    // pDataset.addRow(new Array("Motorista: " + leads[x].CPFMOTORISTA + ' Contato: ' + f_removeCaracteres(leads[x].CONTATO)));
                    pDataset.addRow(new Array(error.toString() + " linha: " + error.lineNumber + ' ORIGEM: ' + leads[x].ORIGEM + " - VISITA: " + leads[x].CODVISITA));
                    printLog('info', 'SQLERR: ' + wSQL);
...
function f_incluirCRM(reg, newDataset) {
    ...
    if (id_user_pipedrive != null) {
        ...
        if (reg.INDCRM == '2') {
            ...
            var datajson = gson.toJson(obj);
            // printLog("info", "JSON: " + datajson);
            newDataset.addRow(new Array("JSON: " + datajson));
...
function f_removeCaracteres(str) {
    ...
    if (str != '' && str != null) {
        var com_acento = "ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝŔÞßàáâãäåæçèéêëìíîïðñòóôőöøùúûüýþÿŕ";
