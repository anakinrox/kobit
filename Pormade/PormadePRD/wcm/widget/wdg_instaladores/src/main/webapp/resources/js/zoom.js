function zoom(componente, idCampoTexto) {
    ...
    if (campo == 'bt_cidade') {
        ...
        if (($('#cod_UF_' + $this.instanceId).val() == "")) {
            FLUIGC.toast({
                message: 'Deve ser selecionado um Estado primeiro',
                type: 'danger'
            });
            return false;
        }
        ...
        modalzoom.open("Cidades",
            ...
            "cidade,Cidades",
            "cidade",
            "endpoint,cidade,uf," + $('#cod_UF_' + $this.instanceId).val(),
            componente, false, "default", null, null,
            "cidade");
    }

    if (campo == 'bt_instalador') {


        if (($('#md_idstatus_atual_' + $this.instanceId).val() == "10") && ($('#cod_instalador_' + $this.instanceId).val() != "")) {
            FLUIGC.toast({
                message: 'Uma instalação já agendada não pode ser alterado o instalador.',
                type: 'danger'
            });
            return false;
        }

        modalzoom.open("Instalador",
            "dsk_instaladores",
            "id,Codigo,nome,Instalador",
            "id,nome",
            "indacao,INSTALADOR",
            componente, false, "default", null, null,
            "nome");
...
function setSelectedZoomItem(selectedItem) {
    ...
    if (campo == "bt_uf") {
        $('#cod_UF_' + $this.instanceId).val(selectedItem.uf);
        $('#UF_' + $this.instanceId).val(selectedItem.nome);
    }

    if (campo == "bt_cidade") {
        $('#cod_cidade_' + $this.instanceId).val(selectedItem.cidade);
        $('#cidade_' + $this.instanceId).val(selectedItem.cidade);
    }

    if (campo == "bt_instalador") {
        $('#cod_instalador_' + $this.instanceId).val(selectedItem.id);
        $('#nome_instalador_' + $this.instanceId).val(selectedItem.nome);
    }

    if (campo == "bt_equipe") {
        $('#nome_equipe_' + $this.instanceId).val(selectedItem.nome);
    }
