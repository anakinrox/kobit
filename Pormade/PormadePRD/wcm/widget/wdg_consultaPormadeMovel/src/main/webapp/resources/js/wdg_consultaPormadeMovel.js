...
var MyWidget = SuperWidget.extend({
    ...
    bindings: {
        ...
        global: {
            ...
            'load-visitasMap': ['click_visitasMap'],
            'load-parceiros': ['click_parceiros'],
            'load-parceirosMap': ['click_parceirosMap'],
    ...
    visitas: function () {
        ...
        var htmlM = '<div class="row"> ' +
            '	<div class="col-md-12"> ' +
            '		<div id="idtableVisitas_' + $this.instanceId + '"></div> ' +
            '	</div> ' +
            '</div>';

        var md_visitas = FLUIGC.modal({
            title: 'Visitas - ' + selected.nomTitulo,
            content: htmlM,
            id: 'fluig-modalVisitas',
            size: 'large',
            actions: []
        }, function (err, data) {
            if (err) {
                ...
                loadDataTable('visitas');
                loadDadosDataTable('visitas', selected.documentid);
            }
        });
    },
    visitasMap: function () {
        ...
        var htmlM = '<div class="row"> ' +
            '		<div id="idmapVisitas_' + $this.instanceId + '" style="width:100%;height:500px;"></div> ' +
            '</div>';

        var md_visitasMap = FLUIGC.modal({
            title: 'Mapa Visitas - ' + selected.nomTitulo,
            content: htmlM,
            id: 'fluig-modalVisitasMap',
            size: 'large',
            actions: []
        }, function (err, data) {
            if (err) {
                ...
                // do something with data
                loadDadosDataTable('visitasMap', selected.documentid);
            }
        });
    },
    parceiros: function () {
        var index = dataTableVisitas.selectedRows()[0];
        var selected = dataTableVisitas.getRow(index);

        var htmlM = '<div class="row"> ' +
            '	<div class="col-md-12"> ' +
            '		<div id="idtableParceiro_' + $this.instanceId + '"></div> ' +
            '	</div> ' +
            '</div>';

        var md_visitas = FLUIGC.modal({
            title: 'Parceiros - ' + selected.local,
            content: htmlM,
            id: 'fluig-modalParceiros',
            size: 'large',
            actions: []
        }, function (err, data) {
            if (err) {
                ...
                loadDataTable('parceiros');
                loadDadosDataTable('parceiros', selected.documentid, selected.local);
            }
        });
    },
    parceirosMap: function () {
        var index = dataTableVisitas.selectedRows()[0];
        var selected = dataTableVisitas.getRow(index);
        ...
        var htmlM = '<div class="row"> ' +
            '		<div id="idmapParceiros_' + $this.instanceId + '" style="width:100%;height:500px;"></div> ' +
            '</div>';

        var md_ParceirosMap = FLUIGC.modal({
            title: 'Mapa Parceiros - ' + selected.local,
            content: htmlM,
            id: 'fluig-modalParceiroMap',
            size: 'large',
            actions: []
        }, function (err, data) {
            if (err) {
                ...
                // do something with data
                loadDadosDataTable('ParceirosMap', selected.documentid, selected.local);
            }
        });
    },
    checklist: function () {
