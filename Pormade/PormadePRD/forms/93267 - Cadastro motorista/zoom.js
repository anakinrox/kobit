...
function zoom(componente, idCampoTexto) {
	...
	if (campo == 'cod_mot_atestado') {

		modalzoom.open("Motivo Atestado",
			"ds_mot_atestado",
			"mot_atestado,Motivo",
			"mot_atestado",
			"",
...
function setSelectedZoomItem(selectedItem) {
	...
	if (campo == 'bt_motorista') {
		...
		$('#cpf').val(selectedItem.mtc_cpf);
		$('#rg').val(selectedItem.mtc_rg);
//		$('#ativo').val(selectedItem.mtc_situacao);
		$('#email').val(selectedItem.mtc_email);
		$('#telefone').val(selectedItem.mtc_telefone.replace(' ',''));
		$('#cnh').val(selectedItem.mtc_numero_cnh);
		$('#whatsapp').val(selectedItem.mtc_whatsapp.replace(' ',''));
		$('#endereco').val(selectedItem.mtc_rua + ', nº '+ selectedItem.mtc_numero_end + ', bairro '+ selectedItem.mtc_bairro + ', cidade '+ selectedItem.mtc_cidade);
	}

	if (campo == 'bt_placa') {
		$('#placa').val(selectedItem.vel_placa);
		$('#modelo').val(selectedItem.vel_modelo);
		
	}

	if (campo == 'cod_tipo_despesa') {
		
		$('#cod_tipo_despesa___'+ seq).val(selectedItem.cod_tipo_despesa);
		$('#tipo_despesa___'+ seq).val(selectedItem.tipo_despesa);

		loadValDespesa(selectedItem.cod_tipo_despesa, seq);
		
	}

	if (campo == 'cod_mot_atestado') {
		$('#den_mot_atestado___'+ seq).val(selectedItem.mot_atestado);
