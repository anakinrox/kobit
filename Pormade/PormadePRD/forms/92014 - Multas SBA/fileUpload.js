...
function loadFile( id ){
	...
	// var num_pasta = $( '#pasta_chamado' ).val();
	folder = '336933';
...
function defNomeFile( id ){
	...
	var num_infracao = $('#num_infracao').val();

	if ( num_infracao == '' ){
		toast('Informe o Número da Infração!','warning');
		return false;
	}
	
	if ( id == 'bt_aif' ){
		descFile =  num_infracao + " - AIF | Auto de infração";
		idDocProc = "doc_aif";
		descDocProc = 'doc_infracao';
	}

	if ( id == 'bt_cpg' ){
		descFile =  num_infracao + " - BLT | Boleto";
		idDocProc = "doc_cpg";
		descDocProc = 'doc_pagamento';
	}
...
function selectFile( files ){
    ...
    $.ajax({
        ...
        success: function(data) {
			...
			$( '#'+descDocProc ).val( descFile+' - '+file.name );
