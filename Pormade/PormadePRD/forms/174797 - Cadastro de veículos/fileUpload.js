...
function openFile( obj ){
	...
	if( obj != null && obj != undefined
	 && $($(obj).parent().children()[0]).val() != ""
	 && $($(obj).parent().children()[0]).val() != undefined ){
			
		var WCMAPI = window.parent.WCMAPI;
		var url = WCMAPI.getServerURL()+WCMAPI.getServerContextURL()+'/p/'+WCMAPI.getOrganizationId()+'/ecmnavigation?app_ecm_navigation_doc='+$($(obj).parent().children()[0]).val();
	    console.log( 'URL..... ', url );
...
function loadFile( id ){
	...
	if( $("#placa").val() == "" ){
		alert('Deve ser informado o veículo.');
		return false;
	}
	
	if( $( '#num_pasta_veiculo' ).val() == "" ){
		if( !confirm("Será criado uma para armazenar os documentos do veículo!") ){
			return false;
		}else{
			criaEstrPastas( id );
		}
...
function defNomeFile( id ){
	...
	if( id == "bt_crlv" ){
		descFile =  "CRLV | Certificado de Registro e Licenciamento de Veículos";
		idDocProc = "doc_crlv";
	...
	if( id == "bt_dut" ){
		descFile =  "DUT | Documento Único de Transferência";
		idDocProc = "doc_dut";
		// modalLabel = ['Data Aposentadoria','data-fluig','dt_aposentadoria'];
		// complemento();
		// folder = $("#fd_dap").val();		
...
function publicaFile( file ){
	...
	$.ajax({
		...
		data: JSON.stringify({
			...
			"parentId": $('#num_pasta_veiculo').val(),
...
function criaEstrPastas( id ){
	...
	var fdm = createFolder( $("#placa").val(), $("#num_pasta_veiculos").val(), false );
	$( '#num_pasta_veiculo' ).val( fdm );
