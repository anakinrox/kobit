...
function validaForm( attr ){
	
	console.log(" Entrei validaForm ", attr);
	var valido = true;
	
	$( 'input, textarea, select' ).each(function () {
		console.log(" while campo ", $(this).attr('id') );
		if ( $('#'+$(this).attr('id') ).attr('readonly') ){
			$( '#'+$(this).attr('id') ).css({'background-color' : '#eee'});
		} else {
		...
		if ($(this).attr('id') == 'rg_ie' && $('#fisico_juridico').val() == 'J'){
			$( '#'+$(this).attr('id') ).css({'background-color' : '#FFFFFF'});
		} else if ($(this).attr('id') == 'especialidade' && $('#profissao').val() != 'INS'){
			$( '#'+$(this).attr('id') ).css({'background-color' : '#FFFFFF'});
		} else if ($(this).attr('id') == 'num_cau_crea' && $('#fisico_juridico').val() == 'J' &&
			(	$('#profissao').val() != 'ARQ' && $('#profissao').val() != 'ENC' && $('#profissao').val() != 'DEI' && $('#profissao').val() != 'TEE')){
			$( '#'+$(this).attr('id') ).css({'background-color' : '#FFFFFF'});
		}
...
function validaAba( aba, attr ){
	
	var valido = true;
	
	$( 'input, textarea, select', $('#'+aba) ).each(function () {
...
function validaCpfCnpj(id,tipo) 
{
	var valor = $("#"+id).val();
	var result = false;
	var label = 'CPF';
	if (tipo=="F"){
		result = validaCPF(valor);
		label = 'CPF';
	}
	if (tipo=="J"){
		result = validaCNPJ(valor);
		label = 'CNPJ';
	}
	
	console.log('Resultado validação',result);
	if( !result ){
  		FLUIGC.toast({message : label+' Invalido. ', type : 'warning' });
 ...
 function validaCEP(valor){
    ...
    if (cep != "") {
		//ExpressÃÂ£o regular para validar o CEP.
		var validacep = /^[0-9]{8}$/;
...
//inscriÃÂ§ÃÂ£o estadual antiga
...
}//fecha funÃÂ§ÃÂ£o CheckIETOantiga
