 ...
 $.get(url, function(data) {
    ...
    if (!("erro" in data)) {
      ...
      //if( $('#den_cidade').val() == "" ){
		 ...
		 constraints.push( DatasetFactory.createConstraint("unaccent(to_ascii(cidade))", retira_acentos(data["localidade"]), retira_acentos(data["localidade"]), ConstraintType.MUST) );
		...
		if ( dataset != null ){
			if( dataset.values.length > 0 ) {
				...
				$('#id_cidade').val( dataset.values[0]['valor_total'] );
  	    	    $('#den_cidade').val( data["localidade"] );
		    	$('#uf').val( data["uf"] );
