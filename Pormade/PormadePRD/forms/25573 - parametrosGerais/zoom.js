...
function zooms(ev){
		...
		//if(retornaSO() != 'Android') {
			...
			if ( ev ==  'condPagto' ) {
				...
				modalzoom.open("pmd_condicao_pagamento", "cdp_descricao,Descrição Condição Pagamento", "condicao_pagamento_cod,cdp_descricao", "Zoom Condição Pagamento", '' , 'condPagto' , "" , "", "" );	
			}
			
			if ( ev ==  'grupoEmail' ) {
				//function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby)
				modalzoom.open("group", "groupId,ID,groupDescription,Descrição", "groupDescription,groupId", "Zoom Grupos", '' , 'grupoEmail' , "" , "", "" );	
			}	
			
			
			if ( ev ==  'grupoExpira' ) {
				//function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby)
				modalzoom.open("group", "groupId,ID,groupDescription,Descrição", "groupDescription,groupId", "Zoom Grupos", '' , 'grupoExpira' , "" , "", "" );	
			}	
			
			if ( ev ==  'grupoVisu' ) {
				//function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby)
				modalzoom.open("group", "groupId,ID,groupDescription,Descrição", "groupDescription,groupId", "Zoom Grupos", '' , 'grupoVisu' , "" , "", "" );	
			}						
		//}else{
			
			/*if ( ev == 'unimed_largura' ) {
				//function(dataset, fields, title, filters, type)
				//var cDatasetReturned1 = DatasetFactory.createConstraint('status_registro', 'A', 'A', ConstraintType.MUST);
				modalzoom2.open("ds_pg_pmd_unidade_medida", "cod_unidade_medida,Código,descricao_unidade_medida,Unidade Medida", "Zoom Unidade Medida", null , 'unimed_largura');
			}*/
...
var modalzoom = (function(){
	...
	return {
		open: function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			if((tipo == "unimed_largura")||(tipo == "unimed_altura")||(tipo == "unimed_espessura")){
				tamModal = "small";
			}else{
				tamModal = "large";
			}
...
function setSelectedZoomItem(selectedItem) {
		...
		if (tipo == "condPagto" ) {
			$("#condicao_pagamento_cod").val(selectedItem.condicao_pagamento_cod);
			$("#cond_pagto_padrao_proposta").val(selectedItem.cdp_descricao);
		}
		
		if (tipo == "grupoEmail" ) {
			$("#id_grupo_email").val(selectedItem.groupId);
			$("#desc_grupo_email").val(selectedItem.groupDescription);
		}
		
		
		if (tipo == "grupoExpira" ) {
			$("#id_grupo_expira").val(selectedItem.groupId);
			$("#desc_grupo_expira").val(selectedItem.groupDescription);
		}
		
		if (tipo == "grupoVisu" ) {
			$("#id_grupo_prop").val(selectedItem.groupId);
			$("#desc_grupo_prop").val(selectedItem.groupDescription);
		}
