...
var wsToken = '66f1f0f57eff1cdfbf60163dfae13f90';
var newDataset;
...
function createDataset(fields, constraints, sortFields) {
    ...
    if (listaConstraits['indacao'] == '') {
        // listaConstraits['indacao'] = 'ADDCOURSE';
        // listaConstraits['idUser'] = '3547';
        // listaConstraits['idCurso'] = '41';

        listaConstraits['indacao'] = 'ADDUSER';
        listaConstraits['fname'] = 'Heloisa';
        listaConstraits['lname'] = 'Teixeira';
        listaConstraits['email'] = 'heloisa29042010@gmail.com';
    ...
    try {
        if (listaConstraits['indacao'] == 'ADDUSER') {
            ...
            while (rsWD.next()) {
                ...
                if (wRretorno.exception == undefined) {
                    ...
                    newDataset.addRow(new Array(wlogin + '', wSenha + '', wcupon + '', wUID + '', 'Usuario cadastrado '));
                } else {
                    if (wRretorno.exception == 'Usuario ja cadastrado') {
                        newDataset.addRow(new Array('', '', '', wRretorno.id, wRretorno.exception));
                    } else {
                        newDataset.addRow(new Array('', '', '', '', 'Usuario não cadastrado'));
                    }
                }
            }
        ...
        if (listaConstraits['indacao'] == 'ADDCOURSE') {
            newDataset.addColumn('STATUS');
            var retorno = f_incluiCurso(listaConstraits['idUser'], listaConstraits['idCurso']);
            newDataset.addRow(new Array('Incluido no Curso: ' + retorno));
        }
        ...
        if (listaConstraits['indacao'] == 'FINDCOURSE') {
            ...
            //log.info('codigo do curso ' + listaConstraits['idCurso']);
            //log.info('Codigo do usuario: ' + listaConstraits['idUser']);

            // var wRetorno = f_consultaConclusao(listaConstraits['idUser'], listaConstraits['idCurso']);
            var wRetorno = f_consultaConclusaoD(listaConstraits['idUser'], listaConstraits['idCurso']);
            ...
            newDataset.addRow(new Array(listaConstraits['idUser'] + '', listaConstraits['idCurso'] + '', wIndConclusao, wPercConcluido + ''));
        }
    } catch (error) {
...
function f_incluiUsario(pUser, pSenha, pFName, pLName, pEmail) {
    var wEndpoint = "/fluig/api.php";
    var clientService = fluigAPI.getAuthorizeClientService();
    var data = {
        companyId: (getValue('WKCompany') || '1') + '',
        serviceCode: 'moodle',
        endpoint: wEndpoint,
        timeoutService: '240',
        method: 'POST'
    ...
    log.info('JSON requisição: ' + jj)
    log.info('Retorno da API Moodle: ' + vo.getResult())
...
function f_incluiUsarioD(pUser, pSenha, pFName, pLName, pEmail) {
    var wEndpoint = "/webservice/rest/server.php?wstoken=" + wsToken + "&wsfunction=core_user_get_users&moodlewsrestformat=json&criteria[0][key]=email&criteria[0][value]=" + pEmail;
    var clientService = fluigAPI.getAuthorizeClientService();
    var data = {
        companyId: (getValue('WKCompany') || '1') + '',
        serviceCode: 'moodle',
...
function f_consultaConclusaoD(pUserID, pCursoID) {

    var wEndpoint = "/webservice/rest/server.php?wstoken=" + wsToken + "&wsfunction=core_completion_get_activities_completion_status&moodlewsrestformat=json&userid=" + pUserID + "&courseid=" + pCursoID;
    var clientService = fluigAPI.getAuthorizeClientService();
    var data = {
        companyId: (getValue('WKCompany') || '1') + '',
        serviceCode: 'moodle',
