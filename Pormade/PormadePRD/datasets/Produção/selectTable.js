	    ...
	    if (fields != null) {
	    	for (var i = 0; i < fields.length; i++) {
	    		if ( fields[i] != 'distinct' ){
	    			...
	    			dataset.addColumn( fields[i].replace('___int___','') );
	    		}else{
	    			distinct = 'distinct';
	    		}
	    	}
...
//	    	    				   log.info( '###### entrei ___int___ campo'+ campo);
	    	    			   ...
	    	    			   if ( campo.indexOf('___in___') >= 0 ){
	    	    				   where += " and "+ campo.replace('___in___','') +" "+
	    	    				   	...
	    	    				   	where += " and "+campo +" "+
	    	    				   			 " "+not+" like "+ upper_lower +"( '%"+ constraints[i].initialValue.replaceAll( "'", "''" ) +"%' ) ";
	    	    			   }else{
	    	    				   if( campoInt ){
		    	    				   	where += " and  "+ campo +" "+
	    	    				   		...
	    	    				   		where += " and "+ campo +" "+
	    	    				   			" "+not+" between "+ upper_lower +"( '"+ constraints[i].initialValue.replaceAll( "'", "''" ) +"' ) and "+ upper_lower +"( '"+ constraints[i].finalValue.replaceAll( "'", "''" ) +"' )";
	    	    				   }
	    	    			   }
	    			}
	    	}
